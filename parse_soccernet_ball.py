...
def get_args():
    ...
    parser.add_argument("label_dir", type=str, help="Path to the SoccerNet-ball labels")
    parser.add_argument("frame_dir", type=str, help="Path to extracted video frames")
    parser.add_argument(
        "-o", "--out_dir", type=str, help="Path to output parsed dataset"
    )
    ...
    for split in ["train", "val", "test", "challenge"]:
        ...
        for video in videos:
            ...
            for half in (1, 2):
                ...
                #     tqdm(
                #         glob.glob(os.path.join(video_frame_dir, "*.jpg")),
                #         desc=f"renaming frames for {video_frame_dir}",
                #     )
                # ):
                #     idx = int(
                #         os.path.basename(frame_path).split(".")[0].replace("frame", "")
                #     )
                #     shutil.move(
                #         frame_path, os.path.join(video_frame_dir, f"{idx:06d}.jpg")
                #     )

                # if num_frames > 0:
                #     breakpoint()

                video_id = "{}/{}/{}".format(league, season, game)
                ...
                for label in video_labels["annotations"]:
                    ...
                    if half == lhalf:
                        ...
                        if adj_frame >= num_frames:
                            print(
                                "Label past end: {} -- {} < {} -- {}".format(
                                    video_id, num_frames, int(adj_frame), label["label"]
                                )
                            )
            ...
            assert len(video_labels["annotations"]) == num_events, video_label_path
