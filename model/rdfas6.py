...
class RDFAS6(nn.Module):
    def __init__(
        ...
        self.branch_module = BranchModule(
            block_n=block_ns[1],
            emb_c=emb_c,
    ...
    def forward(self, x: torch.Tensor, transpose=True, return_idx=0) -> tuple:
        if transpose:
            x = rearrange(x, "b l c -> b c l")
        mask = torch.ones(x.size(0), 1, x.size(2), device=x.device)
        x, mask = self.emb_module(x, mask)
        x, mask = self.stem_module(x, mask)
        xs, masks = self.branch_module(x, mask)
        if transpose:
            xs = tuple(rearrange(x, "b c l -> b l c") for x in xs)
        if return_idx is not None:
            return xs[return_idx]
        return xs
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> tuple:
        for block in self.blocks:
            ...
            for _ in range(self.recurrent):
                ...
                x_c = F.interpolate(
                    x_c, size=x_t.size(-1), mode="nearest"
                )  # b c l//4 -> b c l
...
class BranchModule(nn.Module):
    def __init__(
        self, block_n: int, emb_c: int, kernel_size: int, drop_path_rate: float
    ):
        super().__init__()
        self.blocks = nn.ModuleList()
        for _ in range(block_n):
...
class MaskedConv1D(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        out_mask = (
            F.interpolate(mask.to(x.dtype), size=T // self.stride, mode="nearest")
            if self.stride > 1
...
if __name__ == "__main__":
    # test model
    b, in_c, hid_c, t = 2, 3200, 512, 64
    model = RDFAS6(input_c=in_c, emb_c=hid_c).cuda()
    x = torch.randn(b, t, in_c).cuda()

    # forward
    xs = model(x, transpose=True)
