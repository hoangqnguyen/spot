...
from model.spotmamba import SpotMamba
...
def evaluate(
    model, dataset, split, classes, save_pred, calc_stats=True, save_scores=True
):
    pred_dict = {}
    ...
    for epoch in range(epoch, num_epochs):
        ...
        if args.criterion == "loss":
            ...
            if epoch >= args.start_val_epoch:
                ...
                val_mAP = evaluate(
                    model, val_data_frames, "VAL", classes, pred_file, save_scores=False
                )
        ...
        losses.append(
            {"epoch": epoch, "train": train_loss, "val": val_loss, "val_mAP": val_mAP}
        )
        if args.save_dir is not None:
            os.makedirs(args.save_dir, exist_ok=True)
