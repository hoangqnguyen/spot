...
def get_args():
    ...
    parser.add_argument('--predict_location', action='store_true',
...
class E2EModel(BaseRGBModel):
    ...
    class Impl(nn.Module):
        ...
        def __init__(self, num_classes, feature_arch, temporal_arch, clip_len,
                     modality, predict_location=False):
            super().__init__()
            is_rgb = modality == 'rgb'
        ...
        def forward(self, x):
            ...
            loc_feat = None
            if self._predict_location:
                loc_feat = self._pred_loc(self.__cnn_features).permute(0, 2, 3, 1) # B*T, H, W, 3
    ...
    def __init__(self, num_classes, feature_arch, temporal_arch, clip_len,
                 modality, device='cuda', predict_location=False, multi_gpu=False):
        self.device = device
        self._multi_gpu = multi_gpu
        self._model = E2EModel.Impl(
            num_classes, feature_arch, temporal_arch, clip_len, modality, predict_location)
        ...
        with torch.no_grad() if optimizer is None else nullcontext():
            ...
            for batch_idx, batch in enumerate(pbar):
                ...
                if self._model._predict_location:
                    target_xy = convert_target_to_prediction_shape(batch["xy"], self._model._P).flatten(0,1)
                ...
                with torch.cuda.amp.autocast():
                    ...
                    for i in range(pred.shape[0]):
                        ...
                        if self._model._predict_location:
                            loss_loc += F.smooth_l1_loss(loc[i].reshape(-1, 3), target_xy[i].to(self.device).reshape(-1, 3))
...
def main(args):
    ...
    model = E2EModel(
        ...
        multi_gpu=args.gpu_parallel, predict_location=args.predict_location)
    ...
    for epoch in range(epoch, num_epochs):
        ...
        print(f'[Epoch {epoch}] Train loss: cls={train_loss_dict["cls"]:0.5f}, loc={train_loss_dict["loc"]:0.5f}, sum={train_loss_dict["sum"]:0.5f} | Val loss: cls={val_loss_dict["cls"]:0.5f}, loc={val_loss_dict["loc"]:0.5f}, sum={val_loss_dict["sum"]:0.5f}')
        ...
        losses.append({
            'epoch': epoch, 'train': train_loss_dict["sum"], 'val': val_loss_dict["sum"],
            'val_mAP': val_mAP})
        if args.save_dir is not None:
            os.makedirs(args.save_dir, exist_ok=True)
