...
def get_args():
    ...
    parser.add_argument(
        "-o", "--out_dir", help="Path to write frames. Dry run if None."
    )
        ...
        while True:
            ...
            if i % stride == 0:
                if not RECALC_FPS_ONLY:
                    ...
                    if out_dir is not None:
                        frame_path = os.path.join(out_dir, "{:06d}.jpg".format(i))
        ...
        for season in os.listdir(league_dir):
            ...
            for game in os.listdir(season_dir):
                ...
                for video_file in os.listdir(game_dir):
                    if video_file.endswith("720p.mp4") | video_file.endswith(
                        "720p.mkv"
                    ):
                        ...
                        worker_args.append(
                            (
                                ...
                                (
                                    os.path.join(out_dir, league, season, game)
                                    if out_dir
                                    else None
                                ),
                                sample_fps,
...
if __name__ == "__main__":
    args = get_args()
    args.out_dir = os.path.join(args.out_dir, f"frames{TARGET_HEIGHT}")
    main(args)
